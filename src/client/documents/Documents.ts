...
export namespace DocUtils {
    ...
    export function MakeLink(source: { doc: Doc, ctx?: Doc }, target: { doc: Doc, ctx?: Doc }, title: string = "", description: string = "", id?: string) {
        ...
        UndoManager.RunInBatch(() => {
            ...
            linkDocProto.targetContext = target.doc;
            linkDocProto.sourceContext = source.doc;

            //truncate linked document names
            var sourceTitle: string = StrCast(source.doc.title);
            var targetTitle: string = StrCast(target.doc.title);
            var length = 11;
