...
export class Main extends React.Component {
    ...
    logout = () => {
        window.location.href = ServerUtils.prepend(RouteStore.logout);
    }

    refresh = () => {
        window.location.href = ServerUtils.prepend(RouteStore.home);
    }

    @action
    SetNorthstarCatalog(ctlog: Catalog) {
    ...
    get miscButtons() {
        ...
        let clearDocuments = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
            if (e.ctrlKey) {
                request.get(ServerUtils.prepend(RouteStore.delete), this.refresh);
            }
        }
        let clearDocsSessionsUsers = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
            if (e.ctrlKey) {
                request.get(ServerUtils.prepend(RouteStore.deleteAll), this.logout);
            }
        }
        ...
        return [
            ...
            // database management
            <button className="clear-db-button" key="clear-db" onClick={clearDocuments}>Clear Database</button>,
            <button className="clear-all-db-button" key="clear-all-db" onClick={clearDocsSessionsUsers}>Clear All</button>,
        ]
    }
    ...
    render() {
        return (
            <div id="main-div">
                ...
                {this.miscButtons}
                <WorkspacesMenu
                    active={this.mainContainer}
                    open={this.openWorkspace}
                    new={this.createNewWorkspace}
                    allWorkspaces={this.userWorkspaces}
                />
                <InkingControl />
            </div>
        );
